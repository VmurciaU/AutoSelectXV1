...
def _slug(s: str, maxlen: int = 120) -> str:
    s = (s or "").strip()
    s = re.sub(r"\s+", " ", s)
    # conserva letras, números, _, -, ., #, /, () para que no mutiles tags/códigos
...
def _ensure_table_uid(df_tab: pd.DataFrame) -> pd.DataFrame:
    ...
    # Si ya viene table_uid desde PC-4, úsalo tal cual; si hay NaN, reconstruye
    if "table_uid" in df_tab.columns:
        mask_nan = df_tab["table_uid"].isna() | (
            df_tab["table_uid"].astype(str).str.len() == 0
        )
        if mask_nan.any():
            df_tab.loc[mask_nan, "table_uid"] = df_tab.loc[mask_nan].apply(
                lambda r: f"{r['doc_id']}::p{int(r['_page'] or 0):03d}::t{int(r['_table_idx'] or 0):02d}",
                axis=1,
            )
        return df_tab
...
# -------------------------
def build_graph(
    df_sections: pd.DataFrame, df_tables: pd.DataFrame, df_pid: pd.DataFrame
    ...
    for df in (df_sections, df_tables, df_pid):
        if not df.empty and "doc_id" in df.columns:
            doc_ids.update(
                df["doc_id"].dropna().astype(str).unique().tolist()
            )
    ...
    if not df_sections.empty:
        for _, r in df_sections.iterrows():
            d = str(r.get("doc_id", ""))
            page = r.get("page")
            page = int(page) if (page is not None and not pd.isna(page)) else None
            s_num = (
                str(r.get("section_number"))
                if pd.notna(r.get("section_number"))
                else ""
            )
            s_title = (
                str(r.get("section_title"))
                if pd.notna(r.get("section_title"))
                else ""
            )
            key_text = s_num if s_num else _slug(s_title, 40)
            sid = _node_id(
                "Section", d, page if page is not None else "NA", key_text
            )
            nodes.append(
                {
                    "id": sid,
    ...
    if not df_tables.empty:
        ...
        # Columnas que nos interesan para el nodo de tabla
        cols_keep = [
            "table_uid",
            "doc_id",
            "_page",
        ...
        for c in cols_keep:
            if c not in df_tables.columns:
                df_tables[c] = (
                    "" if c not in ["_page", "_table_idx", "table_order_in_page"] else 0
                )
        ...
        for _, r in tbl_min.iterrows():
            ...
            pg = (
                int(r["_page"])
                if not (pd.isna(r["_page"]) or r["_page"] == "")
                else 0
            )
            tix = (
                int(r["_table_idx"])
                if not (pd.isna(r["_table_idx"]) or r["_table_idx"] == "")
                else 0
            )
            tu = str(r["table_uid"])
            ...
            nodes.append(
                {
                    ...
                    "table_idx": tix,
                    "table_uid": tu,
                    "section_number_near": sidn,
                    "section_title_near": sitt,
                    "caption_near": cap,
                    "table_order_in_page": ordp,
                }
            )
            edges.append(
                {
        ...
        if value_cols:
            if "_row" in df_tables.columns:
                headers = df_tables[df_tables["_row"] == 1][
                    group_cols + value_cols
                ]
            else:
                headers = df_tables.groupby(group_cols).head(1)[
                    group_cols + value_cols
                ]
            ...
            for _, row in headers.iterrows():
                ...
                for c in value_cols:
                    ...
                    if (
                        not col_name
                        or col_name.lower() in ("nan", "none", "null")
                    ):
                        continue
                    ...
                    edges.append(
                        {
                            "src": tid,
                            "dst": pid_,
                            "type": "TABLE_HAS_PARAM",
                        }
                    )

        # 🆕 SECTION_NEAR_TABLE:
        # enlaza sección cercana a la tabla cuando hay match doc_id + page + section_number_near
        if not df_sections.empty:
            for _, r in tbl_min.iterrows():
                d = str(r["doc_id"])
                pg = (
    ...
    if not df_pid.empty:
        ...
        for _, r in df_pid.iterrows():
            ...
            rule = (
                ...
                if have_rule and pd.notna(r.get("rule"))
                else ""
            )
            evd = (
                str(r.get("evidence", "")).strip()
                if have_evd and pd.notna(r.get("evidence"))
                else ""
            )
            ...
            nid = _node_id(
                "PidRef",
                d,
                page if page is not None else "NA",
                _slug(ref or "ref", 64),
            )
            node_rec = {
                "id": nid,
            ...
            if rule:
                node_rec["rule"] = rule
            if evd:
                node_rec["evidence"] = evd
            nodes.append(node_rec)
            edges.append(
                {
                    "src": _node_id("Doc", d),
                    "dst": nid,
                    "type": "DOC_HAS_PID_PAGE",
                }
            )
...
# -------------------------
def _dump_jsonl(
    nodes: pd.DataFrame, edges: pd.DataFrame, outpath: Path
) -> None:
...
def main():
    ap = argparse.ArgumentParser(
        description="PC-5 — Construcción de grafo mínimo (PC-4 extendido)"
    )
    ap.add_argument(
        "--pc4-dir",
        default=str(DEFAULT_PC4_DIR),
        help="Directorio con salidas de PC-4",
    )
    ap.add_argument(
        "--outdir", default=str(DEFAULT_OUTDIR), help="Salida del grafo"
    )
    args = ap.parse_args()
