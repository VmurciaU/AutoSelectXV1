...
# Si está dentro de scripts/, subimos a la raíz; si no, usamos el propio directorio
ROOT = PARENT.parent if PARENT.name == "scripts" else PARENT
...
# =========================
PID_RULES: Dict[str, Dict[str, List[int]]] = {
...
# Ejemplo típico: FFM/F-S-ME-015
PID_CODE_RE = re.compile(r"[A-Z]{2,}(?:/[A-Z0-9\-]+){1,}")
...
def try_import(module_name: str):
    """
    Intenta importar un módulo de varias formas para ser tolerante
    al lugar desde donde se ejecute el script.
    """
    candidates = [
        module_name,
        f"scripts.{module_name}",
        f"app.{module_name}",
        f"app.pipelines.{module_name}",
    ]
...
def extract_tables_shapes(pdf_page) -> List[Tuple[int, int]]:
    """
    Devuelve lista de (n_rows, n_cols) para cada tabla detectada en la página.
    """
    ...
    try:
        ...
        # No queremos reventar el pipeline por un error en la extracción de tablas
        pass
    return shapes


def is_pid_like_page(
    page_text: str,
    ...
    Adicionalmente:
      - Si no hay tablas o las tablas son pequeñas (<= max_rows x max_cols),
        reforzamos la hipótesis de P&ID.
    """
    evidences: List[str] = []
    text = page_text or ""

    # 1) Buscar patrones de título
    ...
    # Si hay una tabla muy grande tipo HD pero sin indicios de diagrama/nota,
    # asumimos que NO es P&ID (es una hoja de datos densa).
    if has_large and not (has_diagrama or has_note):
        return False, evidences

    # 3) Sin tablas => más probable que sea P&ID (layout de imagen/gráfico)
    if not table_shapes:
        return True, evidences

    # 4) Tablas pequeñas => seguimos permitiendo que sea P&ID embebido
    for r, c in table_shapes:
        if r <= max_rows and c <= max_cols:
            return True, evidences

    # 5) Tablas pero todas más grandes que el umbral permitido
    return False, evidences
...
def read_pc2_clean_text(doc_id: str, page_idx1: int) -> str:
    """
    Lee el texto limpio generado por PC-2. Si no existe, devuelve cadena vacía.
    """
...
def get_pdf_for_doc(doc_id: str) -> Tuple[Path, int, str, str]:
    """
    Resuelve el PDF original para un doc_id a partir de manifest.json
    creado en PC-1.
    """
...
def apply_pid_rules(
    doc_type: str,
    page_idx1: int,
    current_pid_like: bool,
    evidence: List[str],
) -> Tuple[bool, List[str]]:
    """
    Aplica reglas de PID_RULES (por tipo de documento).
    - Permite forzar que ciertas páginas sean P&ID o que nunca lo sean.
    """
...
def dispatch_page(
    ...
    """
    Llama al subpipeline adecuado según pid_like:
      - True  -> pc3_pid_pipeline.process_page(...)
      - False -> pc3_text_tables_pipeline.process_page(...)
    """
    result: Dict[str, Any] = {}
    ...
    if pid_like and pid_mod and hasattr(pid_mod, "process_page"):
        ...
        # Ni hay módulo ni función adecuada
...
def run_pc3_for_doc(
    ...
    """
    Ejecuta todo el flujo PC-3 para un documento:
      - Abre PDF original (PC-1/data)
      - Usa texto limpio de PC-2 si existe
      - Marca páginas P&ID vs normales
      - Despacha cada página al subpipeline correspondiente
      - Genera blocks.json + summary.json
      - Llama a finalize(...) del subpipeline de texto/tablas si existe
    """
    ensure_dir(outdir)
    ...
    with pdfplumber.open(str(input_pdf)) as pdf:
        # Por seguridad, nos ajustamos al número real de páginas
        if n_pages:
            n_pages = min(n_pages, len(pdf.pages))
        else:
        ...
        for idx0 in range(n_pages):
            ...
            # 1) Texto: preferimos el limpio de PC-2; si no está, extraemos del PDF
            ...
            # 3) Heurística P&ID base
            pid_like, evidence = is_pid_like_page(
                page_text,
                shapes,
                pid_max_rows,
                pid_max_cols,
            ...
            # 7) Dispatcher a subpipelines
            dispatch_info = dispatch_page(
                pid_module,
                text_module,
                input_pdf,
                outdir,
            ...
            # 8) Acumulamos número total de tablas que el subpipeline diga que emitió
            if "tables_emitted" in dispatch_info:
                try:
                    summary["tables_total"] += int(dispatch_info["tables_emitted"])
                except Exception:
                    # Si viene en formato raro, lo ignoramos sin romper
                    pass

            # 9) Registro de bloque por página (para blocks.json)
            block: Dict[str, Any] = {
                "page": idx1,
            ...
            # Solo guardamos campos internos de dispatch que empiezan por "__"
    ...
    # 10) Persistir resultados del documento
    ...
    # 11) Consolidación al final del documento (TEXT_sections.csv / TEXT_tables_all.csv)
    doc_meta = {
        "doc_id": doc_id,
        "doc_type": doc_type,
        "file_name": file_name,
...
def list_docs_pc2() -> List[str]:
    """
    Lista de doc_id disponibles en outputs/pc2_clean_pages/.
    """
    ...
    return sorted(
        [
            d.name
            for d in PC2_DIR.iterdir()
            if d.is_dir() and any(d.glob(f"{d.name}_page_*.txt"))
        ]
    )
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="PC-3 — Parse blocks + Dispatcher P&ID (batch/single, genérico)"
    )
    ap.add_argument(
        "--doc-id",
        help=(
            "Procesa un solo documento "
            "(carpeta en outputs/pc2_clean_pages/<doc_id>)"
        ),
    )
    ap.add_argument(
        "--outdir",
        help="Salida si se procesa un solo doc (por defecto: outputs/pc3_blocks/<doc_id>)",
    )
    ...
    # Modo batch sobre todas las carpetas de PC-2
    doc_ids = list_docs_pc2()
    if not doc_ids:
        print(
            "[PC3] No hay carpetas en outputs/pc2_clean_pages/. "
            "Corre PC-1 y PC-2 primero."
        )
        sys.exit(0)
